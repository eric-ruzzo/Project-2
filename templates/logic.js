...
function profile_retrieval(ticker) {
      ...
      d3.json(`/company/${ticker}`).then((tickerInfo) =>{
          ...
          if (!tickerInfo['rating_cnt_strong_buys'][0]) {
              ...
              var holds_rating = '<strong>Sells rating : </strong>' + rating_holds[ind];
...
exp_charts.on("click", function() {
  ...
  // Prevent the page from refreshing
      ...
      d3.json("/exp_charts").then((ticker_info) =>{
      
           var current_ticker = ticker_info["ticker"];
           var current_company = ticker_info["company_name"];
      
           console.log('exp_charts :', ticker_info, current_ticker, current_company);
